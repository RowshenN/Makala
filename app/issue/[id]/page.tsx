...
const page = () => {
  ...
  const handleDownload = () => {
    ...
    link.href = issue.pdf;
    link.download = `${issue.title}.pdf`; // file name
          ...
          <div className="w-1/2 flex flex-col items-baseline justify-center gap-3 ">
            ...
            <div className="w-full bg-[#FFFFFF0A] border border-[#FFFFFF14] rounded-md py-4 px-6 flex items-center justify-between">
              ...
              <button
                ...
                disabled={!issue.pdf}
                className={`rounded-md cursor-pointer py-2 px-4 text-[14px] ${
                  issue.pdf
                    ? "bg-[#C8401A] text-white cursor-pointer"
                    : "bg-gray-400 text-white cursor-not-allowed"
                }`}
              >
                {issue.pdf ? "Download" : "No PDF"}
              </button>
            </div>
            ...
            <div className="w-full bg-[#FFFFFF0A] border border-[#FFFFFF14] rounded-md py-4 px-6 flex items-center justify-between">
              ...
              <button
                onClick={handleCopy}
                className="rounded-md py-2 px-4 border border-[#FFFFFF33] text-[#B3B3B3] font-sans font-normal text-[14px] cursor-pointer "
              >
                {copied ? "Copied!" : "Copy"}
              </button>
            </div>
